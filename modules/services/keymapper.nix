...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."keymapper.conf".text = concatStringsSep "\n\n" (
    ...
    systemd.user.services.keymapper = mkIf cfg.systemd.enable {
      ...
      Service = {
        Type = "exec";
        ExecStart =
          "${cfg.package}/bin/keymapper -u -v -c ${config.xdg.configHome + "/keymapper.conf"}"
          + optionalString (!cfg.systemd.tray) " --no-tray";
        Restart = "always";
      };
